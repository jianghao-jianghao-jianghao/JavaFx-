...
class StudentsDao {
    ...
    fun totalReg(): Int? {
        val sql = "SELECT COUNT(*) from studentapp.Students"
        val result = jdbcTemplate!!.queryForObject(sql, Int::class.java)
        return result.toInt()
    }

    fun totalMales(): Int? {
        val sql = "SELECT COUNT(*) from studentapp.Students where `gender` = 'male'"
        val result = jdbcTemplate!!.queryForObject(sql, Int::class.java)
        return result.toInt()
    }

    fun totalFemales(): Int? {
        val sql = "SELECT COUNT(*) from studentapp.Students where `gender` = 'female'"
        val result = jdbcTemplate!!.queryForObject(sql, Int::class.java)
        return result.toInt()
    }
    ...
    val allStudents: List<Students>
        get() {
            val sql = "select * from studentapp.students"
    ...
    fun saveStudent(student: Students): Boolean {
        val query = "insert into studentapp.students values(?,?,?,?,?,?,?,?,?,?,?,?,?)"
