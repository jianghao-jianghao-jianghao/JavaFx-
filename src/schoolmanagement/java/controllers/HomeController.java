...
//        JFXDialog dialog = new JFXDialog((StackPane) mainPane, content, JFXDialog.DialogTransition.CENTER);
        ...
        content.setHeading(new Text("添加部门"));
        VBox box = new VBox();
        box.setSpacing(15);
        ...
        lastNameField.setPromptText("姓名");
        ...
        mobileNumberField.setPromptText("手机号");
...
//        JFXDialog dialog = new JFXDialog((StackPane) mainPane, content, JFXDialog.DialogTransition.CENTER);
        ...
        addBtn.setOnAction(event -> {
            if (deptNameField.getText().isEmpty() || deptNameField.getText().trim().isEmpty()) {
                Alerts.INSTANCE.jfxAlert(mainPane, "错误", "部门字段不能为空");
                return;
            }
