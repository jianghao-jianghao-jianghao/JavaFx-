...
public class SignUpController {
    ...
    private void validators() {
        ...
        requiredFieldValidator.setMessage("输入必填");
        requiredFieldValidator.setIcon(new ImageView(icon));

        numberValidator.setMessage("仅允许整数值");
    ...
    public void handleSignUpBtn(ActionEvent actionEvent) {
        ...
        if (usernameField.getText().isEmpty() || usernameField.getText().trim().isEmpty() ||
            ...
            Alerts.INSTANCE.jfxBluredAlert(signUpBtn, detailsPane, "错误", "所有字段必须填写");
            return;
        }

        if (!Validators.INSTANCE.isValidEmail(emailField.getText())) {
            Alerts.INSTANCE.jfxBluredAlert(signUpBtn, detailsPane, "错误", "无效的邮箱");
            return;
        }

        if (!Validators.INSTANCE.isNumber(mobileNoField.getText())) {
            Alerts.INSTANCE.jfxBluredAlert(signUpBtn, detailsPane, "错误", "手机号码字段必须为整数");
            return;
        }

        if (usersDao.userExist(usernameField.getText())) {
            Alerts.INSTANCE.jfxBluredAlert(signUpBtn, detailsPane, "错误", "用户已存在");
            return;
        }
        ...
        if (userSaved) {
            Alerts.INSTANCE.jfxBluredAlert(signUpBtn, detailsPane, "错误", "发生了一些错误");
            return;
        }
